...
def makeSegMask(img):
	...
	frame_data = frame_data.permute(2,0,1)
	input_tensor = preprocess(frame_data)
...
if __name__ == '__main__':
    ...
    filenames = os.listdir('height-weight-charts')
    for f in filenames:

        # Read each frame one by one
        img = cv2.imread('./height-weight-charts/' + f)
        ...
        if(not img is None):
            ...
            cv2.imwrite('./silhouettes/' + f,final)
